$PBExportHeader$n_new_target_connectservice.sru
$PBExportComments$Generated Database Connection Service
forward
global type n_new_target_connectservice from nonvisualobject
end type
end forward

global type n_new_target_connectservice from nonvisualobject
end type
global n_new_target_connectservice n_new_target_connectservice
...
Choose Case is_connectfrom
	...
	Case IS_USE_INIFILE						/*  Populate Database Connection from INI file  */
		string ls_inifile = "C:\Users\Admin\Documents\reports\new_target.ini"
		...
		as_logpass		= ProfileString ( ls_inifile, "Database", "LogPassword",	"Pass@1234")
		as_server		= ProfileString ( ls_inifile, "Database", "Servername", "localhost\sqlexpress")
		as_dbparm		= ProfileString ( ls_inifile, "Database", "DBParm", "Database='pb'")
	...
	Case IS_USE_REGISTRY						/*  Populate Database Connection from Registry  */
		...
		If RegistryGet ( ls_registrykey, "DBMS", 		RegString!, as_dbms ) <> 1 Then
			RegistrySet ( ls_registrykey, "DBMS", 		RegString!, "MSOLEDBSQL SQL Server" )
		...
		If RegistryGet ( ls_registrykey, "LogPassword", RegString!, as_logpass ) <> 1 Then
			RegistrySet ( ls_registrykey, "LogPassword", RegString!, "Pass@1234" )
		...
		If RegistryGet ( ls_registrykey, "Servername", 	RegString!, as_server ) <> 1 Then
			RegistrySet ( ls_registrykey, "Servername", 	RegString!, "localhost\sqlexpress" )
			RegistryGet ( ls_registrykey, "Servername", 	RegString!, as_server )
		End If
		If RegistryGet ( ls_registrykey, "DBParm", 		RegString!, as_dbparm ) <> 1 Then
			RegistrySet ( ls_registrykey, "DBParm", 		RegString!, "Database='pb'" )
	...
	Case IS_USE_SCRIPT							/*  Populate Database Connection from Script  */
		as_dbms			= "MSOLEDBSQL SQL Server"
		as_database		= ""
		as_userid		= ""
		as_dbpass		= ""
		as_logid			= "sa"
		as_logpass		= "Pass@1234"
		as_server		= "localhost\sqlexpress"
		as_dbparm		= "Database='pb'"
		as_lock			= ""
		as_autocommit	= "false"
...
on n_new_target_connectservice.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_new_target_connectservice.destroy
